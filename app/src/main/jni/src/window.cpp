...
aboutWindow_c::aboutWindow_c(surface_c & s, graphics_c & g) : window_c(2, 0, 16, 12, s, g) {
...

  {
    ...
    snprintf(title, 200, _("Pushover - %s - About"), "1.zbyl");
...
bool window_c::handleEvent(const SDL_Event & event) {
    ...
    if (event.type == SDL_FINGERDOWN)
    {
        ...
        SDL_Rect windowRect = { int(x * gr.blockX()), int(y * gr.blockY()), int(w * gr.blockX()), int(h * gr.blockY()) };
        if (!SDL_PointInRect(&point, &windowRect)) {
            escapePressed = true;
        }
    }
...
bool listWindow_c::handleEvent(const SDL_Event & event) {
    ...
    if (screenTouched)
    {
        ...
        for (size_t i = 0; i < entries.size(); ++i) {
            ...
            if (!SDL_PointInRect(&point, &e.lastPosition)) {
                continue;
            }
