...
class listWindow_c : public window_c {
  ...
  public:
    typedef struct entry {
        ...
        bool line;  ///< Draw separator after the field.

        SDL_Rect lastPosition; ///< Last drawn position. Empty rect if not drawn.

        entry(std::string t) : text(t), highlight(false), sol(0), line(false), lastPosition(SDL_Rect{0, 0, 0, 0}) {}
    } entry;
