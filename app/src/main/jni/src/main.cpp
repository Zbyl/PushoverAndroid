...
bool playTouching[max_fingers];
float touchX[max_fingers];
float touchY[max_fingers];

extern SDL_Rect gScreenRect;

void resetPlayInput() {
    for (auto& t : playTouching)
        t = false;
}

void handlePlayTouchEvents(const SDL_Event & event) {
    if (event.type == SDL_FINGERDOWN)
    {
        if (event.tfinger.fingerId >= max_fingers)
          return;

        playTouching[event.tfinger.fingerId] = true;
        touchX[event.tfinger.fingerId] = event.tfinger.x;
        touchY[event.tfinger.fingerId] = event.tfinger.y;
    } else if (event.type == SDL_FINGERUP)
    {
        if (event.tfinger.fingerId >= max_fingers)
          return;

        playTouching[event.tfinger.fingerId] = false;
    }
}
...
static unsigned int getKeyMask(void) {
  ...
  if ( keystate[SDL_SCANCODE_SPACE] ) keyMask |= KEY_ACTION;
    ...
    int touchIndex = int(touchX[i] * 3) + int(touchY[i] * 3) * 3;
    if (touchIndex == 0 * 3 + 1) keyMask |= KEY_UP;
...
try {
          ...
          switch (nextState) {
            ...
            case ST_PROFILE_DEL:
              window = getProfileSelector(solved, screen, gr);
                  break;
            case ST_LEVELSET:
              window = getMissionWindow(*levelsetList, screen, gr, selectedMission);
                  break;
            case ST_QUIT:
              window = getQuitWindow(screen, gr);
                  break;
            case ST_LEVELCONF:
            case ST_CONFIG:
            ...
            case ST_LEVEL:
              window = getLevelWindow(levelsetList->getLevelset(selectedMission), solved, screen,
                                      gr, lwindowLevel);
                  lwindowLevel = "";
                  break;
                ...
                switch (dynamic_cast<listWindow_c *>(window)->getSelection()) {
                  case 0:
                    nextState = ST_LEVELSET;
                        break;// select level set
                  case 1:
                    nextState = ST_CONFIG;
                        break;  // open config menu
                  case 2:
                    nextState = ST_PROFILE;
                        break; // open profile selector
                  case 3:
                    nextState = ST_ABOUT;
                        break;   // about window
                  case 4:
            ...
            if (!window) {
              ...
              if (window->isDone()) {
                ...
                if (sel >= ls.getLevelNames().size())
                  ...
                  soundSystem_c::instance()->playMusic(
                          datadir + "/themes/" + l.getTheme() + ".ogg");
                  a.initForLevel();
                }
              }
            }
  ...
  SDL_Log("Exception");
  SDL_Log("Exception: %s", exc.what());
} catch(...) {
  SDL_Log("%s", "Some exception.");
}
  return 0;
}
